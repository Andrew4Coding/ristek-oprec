...
export const TotalExpense: React.FC = () => {
    const context = useContext(UserTransactionsContext);
    const [nominal, setNominal] = useState(0);

    useEffect(() => {
        if (context?.transactionList) {
            const incomeList = context?.tempFilter?.filter(item => item.type === "EXPENSE").map(item => item.amount);
    ...
    return (
        <section className="bg-white w-full h-fit shadow-sectionShadow rounded-sectionCorner p-8 font-Manrope text-section-title font-bold flex flex-col gap-2">
